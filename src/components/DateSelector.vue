...
import { format, eachDayOfInterval, eachWeekOfInterval, startOfMonth, lastDayOfMonth, addDays, isSameMonth, isSameDay } from "date-fns";
...
const focusOut = (event: Event) => {
  ...
  if (newValue instanceof Date && !isNaN(newValue.getTime())) {
    ...
    element.value = date.value === '' ? date.value : format(date.value, 'yyyy-MM-dd');
  }
}
...
<template>
  <div class="date-selector">
    ...
    <div class="date-container" :class="{ focused }">
      <input id="date-input" class="date-input" type="text" placeholder="请输入查询日期"
        :value="date === '' ? date : format(date, 'yyyy-MM-dd')" @focusin="focusIn($event)" @focusout="focusOut($event)"
        @keydown.enter="leave($event)" @keydown.esc="leave($event)" />
      ...
      <div class="date-pick-panel" v-show="focused">
        <div class="date-pick-header">
          <div class="date-pick-year">
            <button class="button-flat button-prev" @mousedown="prevYear($event)" />
            <span class="date-pick-title">{{ format(currMonth, 'yyyy') }}</span>
            <button class="button-flat button-next" @mousedown="nextYear($event)" />
          </div>
          <div class="separator" />
          <div class="date-pick-month">
            <button class="button-flat button-prev" @mousedown="prevMonth($event)" />
            <span class="date-pick-title">{{ format(currMonth, 'MM') }}</span>
            <button class="button-flat button-next" @mousedown="nextMonth($event)" />
          </div>
        </div>
        <table class="date-pick-table">
          ...
          <tbody>
            <tr v-for="week in currWeeks" :key="week[0].getDate()">
              <td v-for="day in week" :key="day.getDate()" :class="{ selected: date && isSameDay(day, date), highlight: isSameMonth(day, currMonth), today: isSameDay(day, new Date()) }" @click="date = day">
                {{ day.getDate() }}
              </td>
            </tr>
          </tbody>
